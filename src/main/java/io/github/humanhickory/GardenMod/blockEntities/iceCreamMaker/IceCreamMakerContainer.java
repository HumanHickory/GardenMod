...
import static io.github.humanhickory.GardenMod.blockEntities.iceCreamMaker.IceCreamMakerBlockEntity.*;

public class IceCreamMakerContainer extends AbstractContainerMenu {
    private final BlockPos pos;

    public IceCreamMakerContainer(int windowId, Player player, BlockPos pos) {
        super(MenuTypeInit.ICE_CREAM_MAKER_CONTAINER.get(), windowId);
        this.pos = pos;
        if (player.level().getBlockEntity(pos) instanceof IceCreamMakerBlockEntity iceCreamMaker) {
            //18 pixels apart
    ...
    //shift click moves to available spot
    @Override
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(player.level(), pos), player, BlockInit.ICE_CREAM_MAKER.get());
    }
}
